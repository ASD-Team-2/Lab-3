...
def benchmark(insert):
    ...
    def insertion():
        start = time.time()
        insert(creating_bst(n))
        end = time.time()
        print("Время вставления: {} секунд.".format(end-start))

    """ def removing():
        start = time.time()
        delete(creating_bst(n))
        end = time.time()
        print('[*] Время удаления: {} секунд.'.format(end-start))

    def searcing():
        start = time.time()
        find(creating_bst(n))
        end = time.time()
        print('[*] Время поиска: {} секунд.'.format(end-start))      
 """
    return insertion
